...
typedef struct
{
	EXTI_GPIO_MAPPING_t EXTI_PIN;		//specifies the external interrupt GPIO Mapping
										//this parameter must be set based on a value @ref EXTI_define

	uint8_t				Trigger_Case;	//specifies the Rising or Falling or both trigger
										//this parameter must be set based on a value @ref EXTI_Trigger_define

	uint8_t				IRQ_EN;			//specifies the IRQ Enabled or disabled
										//this parameter must be set based on a value @ref EXTI_IRQ_define

	void (*P_IRQ_CallBack)(void);		//for the C function() which will be called once the IRQ happen
}EXTI_PinConfig_t;
...
void MCAL_EXTI_GPIO_Update(EXTI_PinConfig_t* EXTI_config);		//you can update the configuration at runtime
